...
namespace Testing.SharpMoku
{
	...
	public class GameTesting
	{
		...
		public void PlayerVsPlayerPutCell()
		{
			MockUi ui = new();
			Board board = new(9);
			Game game = new(ui, board, null, 1, GameMode.PlayerVsPlayer);
			Assert.IsTrue(game.GameState == GameState.NotBegin);

			game.NewGame();
			Assert.IsTrue(game.Board.CurrentTurn == Turn.Black);
			Assert.IsTrue(game.GameState == GameState.Playing);
		...
		public void PlayerVsPlayerWonStatus()
		{
			...
			Assert.IsTrue(game.Board.CurrentTurn == Turn.Black);
			Assert.IsTrue(game.GameState == GameState.End);
		...
		[TestMethod]
		[Ignore("Cannot test PlayerVsBotUndo this due to the way application design")]
		public void PlayerVsBotUndo()
		{
			/*
             * This test is different to BoardTest.Undo
             * Because this test would like to determine the scenario that
             * the game has end then it was undone, what is the board's current turn ?
             * The undo method behaviour is dependent on the game mode
             */
			MockUi ui = new();
			Board board = new(9);
			...
			Game game = new(ui, board, bot, 1, GameMode.PlayerVsBot);
			Assert.IsTrue(game.GameState == GameState.NotBegin);
